function hand = sigdemod(signal,fs,callback,pf)
%SIGDEMOD  Interactive filter design and filtering.
%	SIGDEMOD(SIGNAL) opens a signal filtering tool with
%	the default sampling rate set to 8192 Hz.
%
%	SIGDEMOD(SIGNAL,FS) sets the sampling frequency
%	to FS.  FS must be specified in Hz.
%
%	SIGDEMOD('FILENAME') and SIGDEMOD('FILENAME',BITS) open the
...
%
%	See also SIGEDIT, SIGFILT, VOICEDIT, SPECT2D, SPECT3D
...
elseif isstr(signal),
	...
	elseif ~isempty(find(sigfile == '.')),
		...
		error(['sigdemod: Filename must have an extension' ...
...
if min(size(signal)) ~= 1,
	error('sigdemod: Input arg "signal" must be a 1xN or Nx1 vector.');
end;
...
% add workspace menu
if isempty(sigfile),
	m=workmenu(gf,signal,'sigdecal(''restore'');',commoncall, ...
		'sigdemld','sigdesav');
else,
	m=workmenu(gf,sigfile,'sigdecal(''restore'');',commoncall, ...
		'sigdemld','sigdesav');
end;
...
togmenu(gf,'Filter',items,1,'sigdecal(''setfilter'');');
items = str2mat('Lowpass','Highpass','Bandpass','Stopband');
togmenu(gf,'Filter',items,3,'sigdecal(''settype'');');
uimenu(findmenu(gf,'Filter'),'Label','Display Signal',...
	'Separator','on','CallBack','sigdecal(''killtime'');');

% add FIR window menu
mufirwin(gf,'sigdecal(''drawtrans'');');

% Demod menu
callstr = ['spc_me = get(gcf,''CurrentMenu'');' ...
	'spc_state = get(spc_me,''Checked'');' ...
	'if strcmp(spc_state,''on''), ' ...
	'set(spc_me,''Checked'',''off''); ' ...
	'else, ' ...
	'set(spc_me,''Checked'',''on''); ' ...
	'end;' ...
	'clear spc_me spc_state'];
m=uimenu(gf,'Label','Demod');
uimenu(m,'Label','Signal','CallBack',callstr,'Checked','on');
uimenu(m,'Label','AM','CallBack',callstr,'Checked','on');
uimenu(m,'Label','FM','CallBack',callstr);
uimenu(m,'Label','PM','CallBack',callstr);
set(m,'Enable','on');

% Scale menu
items = str2mat('Linear','Logarithmic');
m=togmenu(gf,'Scale',items,2,'sigdecal(''scale'');');

% Full magnitude addition to Scale menu
uimenu(m,'Label','Full Magnitude','Separator','on',...
  	'CallBack','sigdecal(''fullmag'');');
...
uicontrol(gf,'Style','edit','Units','normal','Horiz','center',...
	...
	'Visible','on','UserData','upper',...
	'CallBack','sigdecal(''upper'');');

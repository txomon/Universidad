%SIGMOLD	Load new vector into SIGMODEL.
%	SIGMOLD retrieves the name of the vector to load
...
%
%	See also SIGMODEL, SIGMOCAL, SIGMOSAV
...
if isempty(find(dog_name == '.')) & isempty(find(dog_name == '#')),
	...
	if exist(dog_name) == 1,
		...
		dog_msg = ['sigmold: Variable ' dog_name ' does not exist!'];
		spcwarn(dog_msg,'OK');
...
if min(size(dog_y)) ~= 1,
	dog_msg = ['sigmold: ' dog_name ' is not a vector!'];
	spcwarn(dog_msg,'OK');
	clear dog_msg
end;

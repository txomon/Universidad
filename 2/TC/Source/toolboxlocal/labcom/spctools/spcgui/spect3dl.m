%SPECT3DL	Used by SPECT3D to load new vector.
%	SPECT3DL retrieves the name of the vector to load
...
%	function.
%
%	See also SPECT3D, SPECT3DS, REQSTR
...
if isempty(find(dog_name == '.')) & isempty(find(dog_name == '#')),
	...
	if exist(dog_name) == 1,
		...
		dog_msg = ['spect3dl: Variable ' dog_name ' does not exist!'];
		spcwarn(dog_msg,'OK');
...
if min(size(dog_y)) ~= 1,
	dog_msg = ['spect3dl: ' dog_name ' is not a vector!'];
	spcwarn(dog_msg,'OK');
	clear dog_msg
end;

% store it
set(finduitx(dog_gf,'Apply 3D'),'UserData',dog_y);

%VOICLOAD	Load new vector into VOICEDIT.
%	VOICLOAD retrieves the name of the vector to load
...
%
%	See also VOICEDIT, VOICCALL, VOICSAVE

%	Dennis W. Brown 2-5-94, DWB 3-8-95
...
if isempty(find(dog_name == '.')) & isempty(find(dog_name == '#')),
	...
	if exist(dog_name) == 1,
		...
		dog_msg = ['voicload: Variable ' dog_name ' does not exist!'];
		spcwarn(dog_msg,'OK');
...
if min(size(dog_y)) ~= 1,
	dog_msg = ['voicload: ' dog_name ' is not a vector!'];
	spcwarn(dog_msg,'OK');
	clear dog_msg
end;

function [h,a] = musicspw(rx,N,arg3,arg4,arg5)
%MUSICSPW Weighted MUltiple SIgnal Classification spectrum estimation.
...
if nargin < 1 | nargin > 5,
    error('musicspw: Invalid number of input arguments...');
end;
...
if min(size(rx)) == 1,
	...
	if nargin == 5,
		if ~isstr(arg5),
			error('musicspw: Correlation method must be a string...');
		else,
			method = arg5;
		end;
	end;
	...
	elseif strcmp(method,'rxxmdcov'),
		...
		error('musicspw: Invalid correlation estimation method specified...');
	end;
	...
	[P,m] = size(rx);
	if P ~= m,
		error('musicspw: Rx must be a square matrix...');
	end;
	...
	elseif nargin == 3,
		...
		error('musicspw: Invalid number of input arguments for Rx form.');
	end;
	...
	if nargin == 2,
		if N > m-1,
			error('musicspw: N must be less the rank of Rx...');
		end;
	end;
end;

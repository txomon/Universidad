function zoompklf(ga,cursor,callback)
% ZOOMPKLF Move cursor to next extrema on left.
%       ZOOMPKLF(H,CURSOR) where H is the axis ZOOMTOOL is
%       active in and CURSOR is the cursor number (1 or 2).
%
%       ZOOMPKLF(H,CURSOR,'CALLBACK') evaluates CALLBACK
%       string after a cursor movement.
%
%       Note: ZOOMPKLF is called after a "<<" push button event.
%
...
if k == 1,
	...
	while (sign(y(k)-y(k+1)) ~= sign(y(k)-y(k-1)))
	    k = k-1;
	    if k <= xlim(1)+1, k = xlim(1); break; end;
	end;
end;

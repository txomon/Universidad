...
case 'Function...'
   Function = getuprop(gcbf, 'Function');
   hListBox = findobj('Tag', 'ListBox');
   Value = get( hListBox, 'Value');
   ListString = get( hListBox, 'String');
   FunctionArgs = getuprop(gcbf, 'FunctionArgs');
   if isempty( FunctionArgs )
      setuprop(gcbf, 'Function Output', feval( Function ) );
   else
      ...
      setuprop(gcbf, 'Function Output', feval( Function, FunctionArgs{:} ) );
   end
case 'Cancel'
...
case 'ListBox'
   if strcmp( get(gcbf, 'SelectionType'), 'open')
      ...
      setuprop(gcbf, 'Function Output',[]);
      FuncEnableTest = getuprop(gcbf, 'FuncEnableTest');
...
otherwise
   ...
   if nButtons == 3,
      setuprop(Dlg, 'Function', Function);
      setuprop(Dlg, 'FunctionArgs', FunctionArgs);
      setuprop(Dlg, 'FuncEnableTest', FuncEnableTest);
      setuprop(Dlg, 'FunctionOutput', []);
   end
   ...
   case 'OK'
      ...
      if nButtons == 3, FunctionOutput = getuprop(Dlg, 'Function Output');, end
   case 'Cancel'
      OK = 0;
      Selection = [];

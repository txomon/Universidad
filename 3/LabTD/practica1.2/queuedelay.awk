	...
	if (event_mode[packet_id] ==1) 
        {
      		...
      		printf("%d %.6f %d %d %.6f %.6f  \n", flow_id[packet_id],  queue_delay, packet_id,packet_length[packet_id],t_Dequeue[packet_id],t_Enqueue[packet_id]);
	}
	if (event_mode[packet_id] ==2) 
        {
		drop_mark=1000000;
		finish_mark=1;
		printf("%d %d %d %d %d %.6f \n", flow_id[packet_id], drop_mark,packet_id ,packet_length[packet_id],finish_mark,t_Enqueue[packet_id]);
	}

...
public class EventsActionBean extends AbstractActionBean implements ValidationErrorHandler {
    ...
    private static final String CONFIRM = "/WEB-INF/jsp/events/ConfirmEvents.jsp";
    ...
    private Map<Integer, Integer> expectedassistants;
    @ValidateNestedProperties({
	...
	this.eventsMapper.updateByPrimaryKey(event);
	...
	this.event = this.eventsMapper.selectByPrimaryKey(id);
	return new ForwardResolution(CONFIRM);
    }

    @DontValidate
    public Resolution confirmDeleteForm() {
	AssistancesExample assistancesExample = new AssistancesExample();

	assistancesExample.createCriteria().andEventEqualTo(this.event.getId());

	this.assistancesMapper.deleteByExample(assistancesExample);

	this.eventsMapper.deleteByPrimaryKey(this.event.getId());

	getContext().getMessages().add(new SimpleMessage("El evento " + this.event.getId()
		+ " ha sido eliminado"));
	return new RedirectResolution(EventsActionBean.class, "viewGrid");
    }

    public Resolution insertForm() {
	this.eventsMapper.insert(event);
	return new RedirectResolution("/Events.action?viewForm=&id=" + this.event.getId());
    }
	...
	this.expectedassistants = new TreeMap<Integer, Integer>();

	for (Events item : this.events) {

	    /*
	     * First, we take which ones have come on time
	     */
	    ...
	    /*
	     * Then, we take all the ones that have come
	     */
	    assistancesExample.createCriteria().
		    andEventEqualTo(item.getId());
	    ...
	    assistancesExample.clear();
	    /*
	     * Finally, we take all the ones that should have come
	     */

	    thepost=postsMapper.selectByPrimaryKey(eventTypesMapper.selectByPrimaryKey(item.getEventType()).getPost());

	    relPostsPeopleExample.or().
		    andPostEqualTo(thepost.getId()).
		    andJoinDateLessThanOrEqualTo(item.getDate()).
		    andOutDateGreaterThan(item.getDate());
	    relPostsPeopleExample.or().
		    andPostEqualTo(thepost.getId()).
		    andJoinDateLessThanOrEqualTo(item.getDate()).
		    andOutDateIsNull();

	    this.expectedassistants.put(
		    item.getId(),
		    this.relPostsPeopleMapper.countByExample(relPostsPeopleExample));

	    relPostsPeopleExample.clear();
	}
    }
     ...
     */
    @ValidationMethod(on = "!insertForm")
    ...
    public Map<Integer, Integer> getExpectedassistants() {
	return expectedassistants;
    }

    public void setExpectedassistants(Map<Integer, Integer> expectedassistants) {
	this.expectedassistants = expectedassistants;
    }
